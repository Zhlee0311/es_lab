...
#define LCD_RD_SET GPIOD->BSRR = 1 << 11 // 读数据	PD12
#define LCD_WR_SET GPIOD->BSRR = 1 << 5	 // 写数据			PD5
#define LCD_RS_SET GPIOD->BSRR = 1 << 4	 // 数据/命令 			PD4

#define LCD_CS_CLR GPIOD->BRR = 1 << 7	// 片选端口  		PD7
#define LCD_RD_CLR GPIOD->BRR = 1 << 11 // 读数据		PD12
#define LCD_WR_CLR GPIOD->BRR = 1 << 5	// 写数据			PD5
#define LCD_RS_CLR GPIOD->BRR = 1 << 4	// 数据/命令			PD4
...
/**
 * @brief 配置LCD输入输出的相关引脚，用于读写数据
 */
void LCD_Configuration(void);

/**
 * @brief 校准触摸屏，获取x坐标的AD最大值、最小值，y坐标的AD最大值、最小值
 */
void Touch_Check(void);

/**
 * @brief 触摸屏中断的初始化，包括引脚配置、EXTI配置、NVIC配置等
 */
